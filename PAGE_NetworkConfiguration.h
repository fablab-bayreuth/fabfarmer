...
<hr>
<strong>W-LAN Verbindungsstatus:</strong><div id="connectionstate">N/A</div>
<hr>
<strong>W-LAN Netzwerke:</strong><br>
<table border="0"  cellspacing="3" style="width:310px" >
<tr><td><div id="networks">Suche...</div></td></tr>
<tr><td align="center"><a href="javascript:GetState()" style="width:150px" class="btn btn--m btn--green">AKTUALISIEREN</a></td></tr>
</table>
...
Bitte warten....Konfiguration und Neustart.
)=====";
...
void send_connection_state_values_html()
{
	...
	else if (WiFi.status() == 1) state = "Keine SSID sichtbar";
	else if (WiFi.status() == 2) state = "Suche abgeschlossen";
	else if (WiFi.status() == 3) state = "Verbunden";
	else if (WiFi.status() == 4) state = "Verbindung fehlgeschlagen";
	else if (WiFi.status() == 5) state = "Verbindung abgebrochen";
	else if (WiFi.status() == 6) state = "Verbindung getrennt";
